...
case class TxSignatures(channelId: ByteVector32,
  ...
  val previousFundingTxSig_opt: Option[ByteVector64] = tlvStream.get[PreviousFundingTxSig].map(_.sig)
}

object TxSignatures {
  def apply(channelId: ByteVector32, tx: Transaction, witnesses: Seq[ScriptWitness], previousFundingSig_opt: Option[ByteVector64]): TxSignatures = TxSignatures(channelId, tx.hash, witnesses, TlvStream[TxSignaturesTlv](previousFundingSig_opt.map(TxSignaturesTlv.PreviousFundingTxSig).toList))
}
