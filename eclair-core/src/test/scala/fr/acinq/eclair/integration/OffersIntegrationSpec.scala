...
import fr.acinq.eclair.wire.protocol.{RouteBlindingEncryptedDataTlv, TlvStream}
import fr.acinq.eclair.{CltvExpiry, EclairImpl, Features, MilliSatoshiLong, PayOfferResponse, randomBytes32, randomKey}
import fr.acinq.eclair.payment.Bolt12InvoiceSpec
...
class OffersIntegrationSpec extends IntegrationSpec {
  ...
  test("simple offer") {
    ...
    val invoiceRequest = invoiceRequestPayload.invoiceRequest_opt.get
    val invoiceRequestReplyPath = invoiceRequestPayload.replyPath_opt.get
    ...
    val encodedInvoice = perHopPayloadCodec.encode(TlvStream(Invoice(invoice.records))).require.bytes
