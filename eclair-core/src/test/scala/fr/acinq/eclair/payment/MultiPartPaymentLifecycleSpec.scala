...
class MultiPartPaymentLifecycleSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("successful first attempt (single part)") { f =>
    ...
    val payment = SendMultiPartPayment(sender.ref, finalAmount, SpontaneousRecipient(e, finalAmount, expiry, randomBytes32()), 1, routeParams.copy(randomize = true))
  ...
  test("successful first attempt (multiple parts)") { f =>
    ...
    val payment = SendMultiPartPayment(sender.ref, 1_200_000 msat, recipient, 1, routeParams.copy(randomize = false))
    sender.send(payFsm, payment)
  ...
  test("successful first attempt (trampoline)") { f =>
    ...
    val payment = SendMultiPartPayment(sender.ref, finalAmount + trampolineFees, recipient, 1, routeParams)
    sender.send(payFsm, payment)
  ...
  test("successful retry") { f =>
    ...
    val payment = SendMultiPartPayment(sender.ref, finalAmount, SpontaneousRecipient(e, finalAmount, expiry, randomBytes32()), 3, routeParams)
    sender.send(payFsm, payment)
  ...
  test("retry with updated routing hints") { f =>
    ...
    val payment = SendMultiPartPayment(sender.ref, finalAmount, recipient, 3, routeParams)
    sender.send(payFsm, payment)
  ...
  test("abort after too many failed attempts") { f =>
    ...
    val payment = SendMultiPartPayment(sender.ref, finalAmount, SpontaneousRecipient(e, finalAmount, expiry, randomBytes32()), 2, routeParams)
    sender.send(payFsm, payment)
    router.expectMsgType[RouteRequest]
  ...
  test("abort if no routes found") { f =>
    ...
    sender.watch(payFsm)
    val payment = SendMultiPartPayment(sender.ref, finalAmount, SpontaneousRecipient(e, finalAmount, expiry, randomBytes32()), 5, routeParams)
    sender.send(payFsm, payment)
    router.expectMsgType[RouteRequest]
    router.send(payFsm, Status.Failure(RouteNotFound))
